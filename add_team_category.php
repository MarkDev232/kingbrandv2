...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($categoryName)) {
        ...
        if ($result) {
            echo "<script>alert('Category added successfully'); location.href='update_team_category.php';</script>";
        } else {
            echo "<script>alert('Failed to add category');
            window.location.href('update_year_category.php')</script>";
        }
    } else {
...
<body>
    <div class="container mt-5">
        <a href="update_Team_category.php" class="btn btn-secondary mb-3">
